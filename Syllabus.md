...
| № з/п     | Тема                             | Основні поняття                                 |
| :-------- | :------------------------------- | :---------------------------------------------- |
| **1-4**   | **Основи вебу та JSX**           | SPA, Virtual DOM, Vite, Babel, JSX              |
| **5-9**   | **Компоненти та локальний стан** | Props, useState, Events, useEffect, Keys        |
| **10-14** | **Архітектура та дані**          | Optimization, Context, Routing, API, TypeScript |
| **15-18** | **Екосистема та майбутнє**       | Testing, Styling, SSR, Next.js, AI Ethics       |

### Модуль 1. Основи декларативного програмування та екосистема розробки

1.  **Еволюція веб-технологій та місце React у сучасній розробці.**
...
2.  **Механізми роботи React: Virtual DOM та Reconciliation.**
...
3.  **Середовище розробки та сучасний інструментарій (Tooling).**
...
4.  **JSX: Синтаксис, обмеження та можливості.**
...
5.  **Функціональні компоненти та пропси (Props).**
...
6.  **Хук useState: Робота з локальним станом.**
...
7.  **Обробка подій та робота з формами.**
...
8.  **Життєвий цикл та хук useEffect.**
...
9.  **Списки, ключі та умовний рендеринг.**
...
10. **Оптимізація продуктивності: useMemo та useCallback.**
...
11. **Глобальне керування станом: Context API.**
...
12. **Маршрутизація у SPA: React Router.**
...
13. **Взаємодія з API та асинхронність.**
...
14. **TypeScript у React: Типізація для надійності.**
...
15. **Тестування React-додатків: Від Unit до E2E.**
...
16. **Стилізація та дизайн-системи.**
...
17. **Next.js та Server-Side Rendering (SSR).**
...
18. **Деплоймент, безпека та етика розробки.**
...
Спрямовані на відпрацювання конкретних алгоритмічних та архітектурних задач у форматі коротких вправ та обговорень. Кожне заняття триває 2 години.

1.  **Налаштування оточення та робота з Git.** На практиці студенти ініціалізують проєкти, налаштовують ESLint та Prettier, а також відпрацьовують флоу роботи з гілками в Git.
2.  **Трансформація масивів даних.** Відпрацювання методів map, filter та reduce для підготовки даних до відображення в React-компонентах.
3.  **Логіка умовного відображення.** Створення складних інтерфейсів, що змінюються залежно від стану (наприклад, багаторівневі меню або таби).
4.  **Валідація введених даних.** Реалізація кастомної логіки валідації для форм без використання зовнішніх бібліотек для глибокого розуміння процесів.
5.  **Побудова ієрархії компонентів.** Практична задача з декомпозиції складного макета на дрібні, перевикористовувані модулі.
6.  **Налагодження коду (Debugging).** Використання React DevTools та точок зупинки (breakpoints) у браузері для пошуку помилок у стані та пропсах.
7.  **Робота з URL-параметрами.** Створення динамічних посилань та обробка фільтрів пошуку через Query String.
8.  **Абстракція бізнес-логіки.** Створення кастомних хуків для повторного використання логіки (наприклад, хук для визначення розміру вікна або стану онлайн/офлайн).
9.  **Аналіз продуктивності.** Використання панелі Profiler у React DevTools для ідентифікації "важких" компонентів та їх оптимізації.

## Лабораторний практикум

| № лаб.  | Назва роботи                     | Технологічний стек              |
| :------ | :------------------------------- | :------------------------------ |
| **1-3** | **Базовий інтерфейс та стан**    | React, Vite, CSS Modules        |
| **4-6** | **Навігація та глобальний стан** | React Router, Context API       |
| **7-9** | **Дані, типізація та реліз**     | Axios, TypeScript, Jest, Vercel |

### Лабораторні роботи (18 годин)

Орієнтовані на створення цілісного веб-додатка (наприклад, соціальної платформи або таск-менеджера).

1.  **Створення каркаса додатка та базових UI-компонентів.** Використання атомарного дизайну для створення кнопок, інпутів та карток.
2.  **Реалізація стрічки новин або списку об'єктів.** Робота з реальними даними, впровадження списків та ключів.
3.  **Інтеграція системи фільтрації та пошуку.** Робота з локальним станом для забезпечення миттєвого відгуку інтерфейсу на дії користувача.
4.  **Розробка багатосторінкової навігації.** Впровадження React Router та створення сторінок профілю та налаштувань.
5.  **Створення системи автентифікації.** Реалізація форм входу/реєстрації та захищених маршрутів (Protected Routes) за допомогою Context API.
6.  **Підключення до зовнішнього API.** Використання JSONPlaceholder або власного бекенду для синхронізації даних додатку.
7.  **Впровадження TypeScript.** Повний рефакторинг коду лабораторних робіт з додаванням типізації для підвищення надійності системи.
8.  **Написання тестів.** Покриття основних функцій додатку (наприклад, додавання коментаря або зміна профілю) юніт-тестами.
9.  **Фінальна збірка та деплоймент.** Оптимізація бандлу та розміщення проєкту в мережі Інтернет з налаштуванням CI/CD пайплайну.

## Самостійна робота та індивідуальні дослідження

1.  **Дослідження екосистеми Next.js (10 годин).** Самостійне вивчення App Router, Server Actions та методів оптимізації зображень.
2.  **Опанування інструментів тестування (8 годин).** Глибоке занурення в інтеграційне тестування та мокінг API за допомогою MSW (Mock Service Worker).
3.  **Вивчення State Management бібліотек (8 годин).** Порівняльний аналіз Redux Toolkit та Zustand. Реалізація невеликого модуля з використанням однієї з бібліотек.
4.  **Розробка власного кастомного хука для складних задач (6 годин).** Наприклад, хук для роботи з WebSockets або IndexedDB.
5.  **Підготовка до захисту проєкту (16 годин).** Написання документації, підготовка презентації та проведення фінального тестування розробленого додатка.
